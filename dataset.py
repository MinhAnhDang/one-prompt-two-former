...
class ISIC2016(Dataset):
    def __init__(self, args, data_path , transform = None, transform_msk = None, mode = 'Training',prompt = 'click', plane = False):
        ...
        self.name_list = df['image'].tolist()
    ...
    def __getitem__(self, index):
        ...
        """Get the images"""
        name = self.name_list[index]
        img_path = os.path.join(self.data_path,'ISBI2016_ISIC_Part1_'+self.mode+'_Data', name)
        
        mask_name = self.label_list[index]
        msk_path = os.path.join(self.data_path,'ISBI2016_ISIC_Part1_'+self.mode+'_GroundTruth', mask_name)
